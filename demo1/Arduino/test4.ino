...
void loop() {
  ...
  if ( precision_start ) {
    Serial.print('\n');
    Serial.print(intError_r);
    if ( data <= 50 ) {
      target = target_temp - data;
    }
    else {
      target = target_temp + ((256 - data));
    }
...
//  if (theta-target <=  3 && theta-target >=  -3){// THIS IF STATEMENT IS SUPER DUPER IMPORTANT!!!!!!!!!!!!
...
//  }
  ...
  if (currMyTime_r >= prevMyTime_r+100){ // sample every 10th of a second
    ...
    if (theta-target <=  2 && theta-target >=  -2){// THIS IF STATEMENT IS SUPER DUPER IMPORTANT!!!!!!!!!!!!
      theta_r = target;                // There is some noise in the system that will cause everything to become unstable
      intError_r = 0;                         // This if statement rejects such noise and keeps the system from running away
    }
...
void MOTOR_MOVE() {
  ...
  if ( (vel <= 100) && (abs(theta - target) >= 2) ) {
    vel = 150;
  }

  
  md.setM1Speed(vel * kp_again);
  md.setM2Speed(vel * (.9) * kp_again);
...
void PI_READ(int byteCount) { 
  ...
  // temp key makes this shit go only once for memory
  if ( temp_key ) {
    
    target_temp = theta; // target_temp prevents recursion of target
    temp_key = 0;
    thirty_deg = 0;
    wait = millis();
    key = 4;
