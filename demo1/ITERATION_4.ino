...
void RIGHT_MOTOR_MOVE(){
  ...
  if (theta-target <=  10 && theta_r-target_ >=  -10){// THIS IF STATEMENT IS SUPER DUPER IMPORTANT!!!!!!!!!!!!
  ...
  if (currMyTime_r >= prevMyTime_r+100){ // sample every 10th of a second
    ...
    motorVal_r = double(( target_r-theta_r )) * Kp + (Ki * double(intError_r)); //calculate the motor val
...
void LEFT_MOTOR_MOVE(){
  ...
  /*
    ...
    if ( current - target is negative , we move right
    left go big right go small
    left theta is positive
    right theta is negative

    else 
    left go small right go big
    left theta is negative
    right theta is positive  
  ...
  */
  ...
  if (theta_l-target <=  10 && theta_l-target_ >=  -10){// THIS IF STATEMENT IS SUPER DUPER IMPORTANT!!!!!!!!!!!!
  ...
  if (currMyTime_l >= prevMyTime_l+100){ // sample every 10th of a second
    ...
    // takes in values from -400 to 400
    // uses a built in PWM to control the voltage supplied to the motor
    motorVal_l = double(( target_l-theta_l )) * Kp + (Ki * double(intError_l)); //calculate the motor val
