...
#            nextMarker = 1
        try:
            if '1' not in data and '2' not in data and '3' not in data:
                #print('hi')
                data = ''
                for i in range(0,3):
                    #print('test')
...
markerSize = 4.97 # in cm, 5.39 for demo1 marker, 5.08 for the one we printed out, IMPORTANT: CHANGE ON DAY OF DEMO # 4.5 cm
    ...
    #                continue
                ...
                for (markerCorner, markerID) in zip(corners, ids): # Loop through the seen markers
                    if markerID == currID:  # Isolate information for desired marker
                        ...
                        distFromCamToMarker = round( math.sqrt(math.pow(z,2)), 1)  # + math.pow(y,2))
