...
markerSize = 5.38 # in cm, 5.39 for demo1 marker, 5.08 for the one we printed out, IMPORTANT: CHANGE ON DAY OF DEMO # 4.5 cm
...
guidance = [1,2,3,4,5,6,-1] # sequence
    ...
    #                continue
                ...
                for (markerCorner, markerID) in zip(corners, ids): # Loop through the seen markers
                    if markerID == currID:  # Isolate information for desired marker
                        ...
                        distFromCamToMarker = round( math.sqrt(math.pow(z,2)) * 0.93, 1) + 0.1    # + math.pow(y,2))
...
#                        distAvg = []
#                        distAvg = [0 for i in range(20)]
#                        for i in range (20):
#                            distFromCamToMarker = round( math.sqrt(math.pow(z,2)) * 0.93, 1) + 0.1    # + math.pow(y,2))
#                            print(distFromCamToMarker)
#                            distAvg[i] = distFromCamToMarker
#                        #distAvg = distAvg.sort()
#                        print(distAvg)
                        #distFromCamToMarker = distAvg[10]
