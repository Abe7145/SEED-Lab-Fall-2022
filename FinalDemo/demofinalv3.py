...
markerSize = 6.00 # in cm, 5.39 for demo1 marker, 5.08 for the one we printed out, IMPORTANT: CHANGE ON DAY OF DEMO # 4.5 cm
    ...
    #                continue
                ...
                for (markerCorner, markerID) in zip(corners, ids): # Loop through the seen markers
                    if markerID == currID:  # Isolate information for desired marker
                        ...
                        distFromCamToMarker = round( math.sqrt(math.pow(z,2) + math.pow(y,2)) , 1) + 0.2 #* math.cos(actAngle * (math.pi/ 180))
