...
#define DEMO_TWO 9
#define SECRETCODE 10
#define WAIT_TWO 11
#define PRE_PRECISION 12
...
///////////////////////////////////////////////////////////////////////////////////////
const double Ki = .33;//0;//.33;//.33
const double Kp = 3;//4.5;//3
...
void loop() {
  ...
  switch ( Curr_State ) {
    ...
    case ROUGH:
      //if ( (abs(target - theta) <= 10) || abs(time_control - millis() ) >= 3000 ) {
    ...
    case FIRST_STOP:
      ...
      if ( request || (abs(time_control - millis()) >= 10000)) {
    ...
    case GET_POSITION_ONE:
      ...
      if ( reading == 1 || (abs(time_control - millis()) >= 10000)) {
    ...
    case MOVE_FORWARD: // case that once aruco is detected, robot moves towards it
      ...
      if ( big_boy_error <= 5 ) {
        ...
        Next_State = SECRETCODE;
        super_secret_code = 456;
      }
      break;
    ...
    case WAIT_TWO:
      super_secret_code = 0;
      if (abs(time_control - millis()) >= 5000) {
...
void RIGHT_MOTOR_MOVE(){
  ...
  if (currMyTime_r >= prevMyTime_r+100){ // sample every 10th of a second
    ...
    if ( ( motorVal_r >= 50 ) && (Curr_State == 1 || Curr_State == 2) ) {//50->75
      motorVal_r = 400;
    }
...
void LEFT_MOTOR_MOVE(){
  ...
  if (currMyTime_l >= prevMyTime_l+100){ // sample every 10th of a second
    ...
    if ( (motorVal_l >= 50) && (Curr_State == 1 || Curr_State == 2) ) {//50->75
      motorVal_l = 400;
    }
